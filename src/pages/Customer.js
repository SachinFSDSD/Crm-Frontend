...
function Customer() {
  ...
  const onTicketUpdate = (e) => {
    ...
    else if (e.target.name === "status") selectedTicket.status = e.target.value;
  ...
  const updateTicket = (e) => {
    ...
    axios
      .put(
        BASE_URL + "/crm/api/v1/tickets/" + selectedTicket.id,
        selectedTicket,
        {
          headers: {
  ...
  const createTicket = (e) => {
    ...
    axios
      .post(BASE_URL + "/crm/api/v1/tickets", data, {
        headers: {
          "x-access-token": localStorage.getItem("token"),
        },
      })
      ...
      .catch(function (error) {
        if (error.status && error.status === 400) setMessage(error.message);
  ...
  return (
    <div className="bg-light">
      ...
      <div className="container vh-100">
        ...
        <TicketTable editTicket={editTicket} ticketDetails={ticketDetails} />
        <input
          type="submit"
